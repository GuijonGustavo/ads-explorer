...
class author:
    ...
    def papers(self):
        ...
        if self.all_papers == None:
            ...
            if not isinstance(name, str) and name != None:
                papers_name = list(ads.SearchQuery(author=name[0], fl=self.fl, **self.kw))
                papers_name_id = [x.id for x in papers_name]
                for kname in range(1,len(name)):
                    papers_kname = list(ads.SearchQuery(author=name[kname], fl=self.fl, **self.kw))
                    for pkn in papers_kname:
                        if pkn.id not in papers_name_id:
                            papers_name.append(pkn)
                            papers_name_id.append(pkn.id)
            elif name != None:
                papers_name = list(ads.SearchQuery(author=name,fl=self.fl, **self.kw))
                papers_name_id = [x.id for x in papers_name]
            else:
                papers_name = []
                papers_name_id = []
            ...
            if orcid != None:
                papers_orcid = list(ads.SearchQuery(orcid=orcid,fl=self.fl, **self.kw))
    ...
    def papers_first(self):
        ...
        if self.papers_fauth == None:
            ...
            orcid = self.orcid
            if not isinstance(name, str) and name != None:
                papers_name = list(ads.SearchQuery(first_author=name[0],fl=self.fl, **self.kw))
                papers_name_id = [x.id for x in papers_name]
                for kname in range(1,len(name)):
                    papers_kname = list(ads.SearchQuery(first_author=name[kname],fl=self.fl, **self.kw))
                    for pkn in papers_kname:
                        if pkn.id not in papers_name_id:
                            papers_name.append(pkn)
                            papers_name_id.append(pkn.id)
            elif name != None:
                papers_name = list(ads.SearchQuery(first_author=name,fl=self.fl, **self.kw))
                papers_name_id = [x.id for x in papers_name]
            else:
                papers_name = []
                papers_name_id = []
            ...
            if orcid != None and name == None:
                ...
                papers_orcid = list(ads.SearchQuery(first_name=name, orcid=orcid, fl=self.fl, **self.kw))
    ...
    def papers_not_first(self):
        ...
        if self.papers_nauth == None:
            ...
            orcid = self.orcid
            if not isinstance(name, str) and name != None:
                papers_name_1 = list(ads.SearchQuery(first_author=name,fl=self.fl, **self.kw))
                papers_name_1_id = [x.id for x in papers_name_1]
                papers_name_2 = list(ads.SearchQuery(author=name,fl=self.fl, **self.kw))
                ...
                for kname in range(1,len(name)):
                    papers_kname_1 = list(ads.SearchQuery(first_author=name,fl=self.fl, **self.kw))
                    papers_kname_1_id = [x.id for x in papers_name_1]

                    papers_kname_2 = list(ads.SearchQuery(author=name,fl=self.fl, **self.kw))
             ...
             #   print('Sorry, ADS does not have the functionality to search by ORCID and specifying author position, please add a name to be able to do this')
              #  return None
                ...
                papers_orcid_1 = list(ads.SearchQuery(orcid=orcid,fl=self.fl, **self.kw))
                papers_orcid_1_id = [x.id for x in papers_orcid_1]

                papers_orcid_2 = list(ads.SearchQuery(orcid=orcid,fl=self.fl, **self.kw))
    ...
    def collaboration(self,author_2):
        ...
        print(f'Collaboration stats between {self.name} and {author_2.name}:') #CHANGE TO AUTHOR LABELS
